...
/**
 * Main file of the "fractal" template for DokuWiki
 *
 *
...
//load userdefined js?
if (tpl_getConf("fractal_loaduserjs") && file_exists(DOKU_TPLINC."user/user.js")){
    echo "<script type=\"text/javascript\" charset=\"utf-8\" src=\"".DOKU_TPL."user/user.js\"></script>\n";
}
?>
<!--[if lte IE 8]><link rel="stylesheet" media="all" type="text/css" href="<?php echo DOKU_TPL; ?>css/fractal_screen_iehacks.css" /><![endif]-->
    ...
    <div class="matrixHeader" >
      ...
      <?php
          ...
          if($ACT === "show"){           // check if we are not on an admin page
            ...
            $contents = str_replace("network_files", "./lib/tpl/fractal/js/network_files", $contents);
    ...
    <div class="dokuwiki">
        ...
        <div id="tmpl_footer">
            ...
            <div id="tmpl_footer_actlinksright">
                <?php
                if (!empty($loginname) ||
                    !tpl_getConf("fractal_hideadminlinksfromanon")){
                    echo "[&#160;";
            ...
            <div id="tmpl_footer_metainfo">
                <!-- Please do NOT remove the following fractal and/or DokuWiki link/notice. Thank you. :-) -->
                <a href="https://www.dokuwiki.org/template:fractal" target="_blank"<?php echo ((cleanID(getID()) === "start") ? "" : " rel=\"nofollow\"") ?>>fractal on DW</a> under the hood
                <?php
                if(!empty($INFO["exists"]) &&
                   tpl_getConf("fractal_showpageinfo")) {
                    echo " &#160;|&#160; ";
                    tpl_pageinfo();
                }
                if (!empty($loginname)){
                ...
                if (tpl_getConf("fractal_footer")){
                    if (empty($conf["useacl"]) ||
                        auth_quickaclcheck(cleanID(tpl_getConf("fractal_footer_location")))){ //current user got access?
                        //get the rendered content of the defined wiki article to use as custom content
                        $interim = tpl_include_page(tpl_getConf("fractal_footer_location"), false);
                        if ($interim === "" ||
                            $interim === false){
                            //show creation/edit link if the defined page got no content
                            echo "<p>[&#160;";
                            tpl_pagelink(tpl_getConf("fractal_footer_location"), hsc($lang["fractal_fillplaceholder"]." (".tpl_getConf("fractal_footer_location").")"));
                            echo "&#160;]</p>";
                        }else{
            ...
            //copyright notice
            if (tpl_getConf("fractal_copyright")){
                echo "\n            <div id=\"licenseinfo\">\n                ";
                if (tpl_getConf("fractal_copyright_default")){
                    tpl_license(false);
                }else{
                    if (empty($conf["useacl"]) ||
                        auth_quickaclcheck(cleanID(tpl_getConf("fractal_copyright_location")))){ //current user got access?
                        //get the rendered content of the defined wiki article to use as custom notice
                        $interim = tpl_include_page(tpl_getConf("fractal_copyright_location"), false);
                        if ($interim === "" ||
                            $interim === false){
                            //show creation/edit link if the defined page got no content
                            echo "[&#160;";
                            tpl_pagelink(tpl_getConf("fractal_copyright_location"), hsc($lang["fractal_fillplaceholder"]." (".tpl_getConf("fractal_copyright_location").")"));
                            echo "&#160;]<br />";
                        }else{
                            //show the rendered page content
                            echo $interim;
