...
/**
 * Main file of the "fractal10" template for DokuWiki
 *
 *
...
//load userdefined js?
if (tpl_getConf("fractal10_loaduserjs") && file_exists(DOKU_TPLINC."user/user.js")){
    echo "<script type=\"text/javascript\" charset=\"utf-8\" src=\"".DOKU_TPL."user/user.js\"></script>\n";
}
?>
<!--[if lte IE 8]><link rel="stylesheet" media="all" type="text/css" href="<?php echo DOKU_TPL; ?>css/fractal10_screen_iehacks.css" /><![endif]-->
...
<div id="pagewrap"<?php
    ...
    <div id="tmpl_header">
        ...
        <div id="tmpl_header_right">
            ...
            if (tpl_getConf("fractal10_headernav")){
                echo "<div id=\"tmpl_header_nav\">\n                ";
                //we have to show a custom navigation
                if (empty($conf["useacl"]) ||
                    auth_quickaclcheck(cleanID(tpl_getConf("fractal10_headernav_location")))){ //current user got access?
                    //get the rendered content of the defined wiki article to use as custom navigation
                    $interim = tpl_include_page(tpl_getConf("fractal10_headernav_location"), false);
                    if ($interim === "" ||
                        $interim === false){
                        //show creation/edit link if the defined page got no content
                        echo "[&#160;";
                        tpl_pagelink(tpl_getConf("fractal10_headernav_location"), hsc($lang["fractal10_fillplaceholder"]." (".tpl_getConf("fractal10_headernav_location").")"));
                        echo "&#160;]<br />";
                    }else{
...
<div class="clearer"></div>
        ...
        <div id="tmpl_footer">
            ...
            <div id="tmpl_footer_actlinksright">
                <?php
                if (!empty($loginname) ||
                    !tpl_getConf("fractal10_hideadminlinksfromanon")){
                    echo "[&#160;";
            ...
            <div id="tmpl_footer_metainfo">
                <!-- Please do NOT remove the following fractal10 and/or DokuWiki link/notice. Thank you. :-) -->
                <a href="https://www.dokuwiki.org/template:fractal10" target="_blank"<?php echo ((cleanID(getID()) === "start") ? "" : " rel=\"nofollow\"") ?>>fractal10 on DW</a> under the hood
                <?php
                if(!empty($INFO["exists"]) &&
                   tpl_getConf("fractal10_showpageinfo")) {
                    echo " &#160;|&#160; ";
                    tpl_pageinfo();
                }
                if (!empty($loginname)){
                ...
                if (tpl_getConf("fractal10_footer")){
                    if (empty($conf["useacl"]) ||
                        auth_quickaclcheck(cleanID(tpl_getConf("fractal10_footer_location")))){ //current user got access?
                        //get the rendered content of the defined wiki article to use as custom content
                        $interim = tpl_include_page(tpl_getConf("fractal10_footer_location"), false);
                        if ($interim === "" ||
                            $interim === false){
                            //show creation/edit link if the defined page got no content
                            echo "<p>[&#160;";
                            tpl_pagelink(tpl_getConf("fractal10_footer_location"), hsc($lang["fractal10_fillplaceholder"]." (".tpl_getConf("fractal10_footer_location").")"));
                            echo "&#160;]</p>";
                        }else{
                            //show the rendered page content
                            echo $interim;
            ...
            //copyright notice
            if (tpl_getConf("fractal10_copyright")){
                echo "\n            <div id=\"licenseinfo\">\n                ";
                if (tpl_getConf("fractal10_copyright_default")){
                    tpl_license(false);
                }else{
                    if (empty($conf["useacl"]) ||
                        auth_quickaclcheck(cleanID(tpl_getConf("fractal10_copyright_location")))){ //current user got access?
                        //get the rendered content of the defined wiki article to use as custom notice
                        $interim = tpl_include_page(tpl_getConf("fractal10_copyright_location"), false);
                        if ($interim === "" ||
                            $interim === false){
                            //show creation/edit link if the defined page got no content
                            echo "[&#160;";
                            tpl_pagelink(tpl_getConf("fractal10_copyright_location"), hsc($lang["fractal10_fillplaceholder"]." (".tpl_getConf("fractal10_copyright_location").")"));
                            echo "&#160;]<br />";
                        }else{
                            //show the rendered page content
                            echo $interim;
